    ...
    pub fn uniform_answer(spec: &QuestionSpec) -> RawAnswer {
        match spec {
            ...
            QuestionSpec::Choice { criteria, .. } => {
                ...
                let probabilities: BTreeMap<String, f64> = criteria.iter().map(|(k, _)| (k.clone(), p)).collect();
                RawAnswer::Choice {
            ...
            QuestionSpec::Score { criteria, .. } => {
                let n = criteria.len();
                let p = 1.0 / n as f64;
                let probabilities: BTreeMap<String, f64> = (0..n).map(|i| (i.to_string(), p)).collect();
...
pub mod answers {
    ...
    pub fn choice<'a>(pairs: impl IntoIterator<Item = (&'a str, f64)>) -> RawAnswer {
        let mut probabilities: BTreeMap<String, f64> = pairs.into_iter().map(|(k, p)| (k.to_string(), p)).collect();
    ...
    pub fn score(level_probs: &[f64], legend: &[&str]) -> RawAnswer {
        ...
        let probs: Vec<f64> = level_probs
            .iter()
            .map(|p| if total > 0.0 { p / total } else { *p })
            .collect();
