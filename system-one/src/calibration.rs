...
impl CalibrationReport {
    ...
    pub fn from_pairs<I: IntoIterator<Item = (f64, bool)>>(pairs: I, n_bins: usize) -> Self {
        ...
        for (i, (c, sp, spos)) in sums.iter().enumerate() {
            ...
            let (mean_p, frac_pos) = if *c > 0 {
                (sp / *c as f64, *spos as f64 / *c as f64)
            } else {
                (f64::NAN, f64::NAN)
            };
            if *c > 0 {
    ...
    pub fn from_records<'a, I: IntoIterator<Item = &'a Record>>(records: I, question: &str, n_bins: usize) -> Self {
        ...
        for r in records {
            let Some(outcome) = r.outcome.as_ref().and_then(|o| o.get(question)) else {
                continue;
            };
            let Some(ans) = r.answers.answers.get(question) else {
                continue;
            };
            match ans {
                RawAnswer::Noul { noul, .. } => {
